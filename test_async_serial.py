...
class AsyncSerialTestSuite(unittest.TestCase):
    ...
    def test_get_content(self):
        result = pr._Protocol.get_protocol().header
        print(result)
    def test_set_content(self):
        result = pr._Protocol.get_protocol().set_content(1,2,3,4,5,6)
        print(result)
    def test_get_bytes(self):
        data = b''
        result = pr._Protocol.get_protocol().produce_package(len(data), 1, 2, 3, 4, data)
        self.assertEqual(result,b'\xcd\xab\x00\x00\x00\x01\x02\x00\x03\x04b')

    def test_get_header_content(self):
        result = pr._Protocol.get_protocol().get_header_content(b'\xcd\xab\x00\x00\x01\x02\x03\x00\x04\x00\x05\x006Q')
        print(result)

    def test_sub_get_bytes(self):
        data = b''
        result = pr.Sub_Protocol.get_sub_protocol().produce_sub_package(len(data), 2, 3, 4, 5, data)
